...
namespace WeatherUtilities
{
    public class WeatherCalculations
    {
        ...
        {
            if (temperatureF < 80)
                return temperatureF;  // Heat index only applies when T >= 80°F

            double HI = -42.379 + 2.04901523 * temperatureF + 10.14333127 * relativeHumidity
        ...
        public double CalculateWindChill(double temperatureF, double windSpeedMph)
        {
            if (temperatureF > 50 || windSpeedMph < 3)
                return temperatureF;  // Wind chill only applies when T <= 50°F and wind >= 3 mph

            double windChill = 35.74 + 0.6215 * temperatureF - 35.75 * Math.Pow(windSpeedMph, 0.16)
        ...
        public double CalculateDewPoint(double temperatureC, double relativeHumidity)
        {
            ...
            double alpha = ((a * temperatureC) / (b + temperatureC)) + Math.Log(relativeHumidity / 100.0);
